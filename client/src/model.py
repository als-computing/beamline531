...
class BeamlineComponents:
    ...
    def find_component(self, base_id: "str" = None):
        ...
        try:
            ...
            logging.error(
                f"Base id {base_id} was not found among the list of components."
            )
            return None
...
class QServer(BaseModel):
    ...
    def get_status(self):
        ...
        if response.status_code != 200:
            logging.error(
                f"Could not retrieve status {response.status_code}:{response.json()}"
            )
    ...
    def open_env(self):
        ...
        """
        response = requests.post(
            f"{self.api_url}/environment/open",
            headers={"Authorization": f"ApiKey {self.api_key}"},
        )
        if response.status_code != 200:
            logging.error(
                f"Could not open new environment {response.status_code}:{response.json()}"
            )
        return response.json()
    ...
    def close_env(self):
        ...
        """
        response = requests.post(
            f"{self.api_url}/environment/close",
            headers={"Authorization": f"ApiKey {self.api_key}"},
        )
        if response.status_code != 200:
            logging.error(
                f"Could not close environment {response.status_code}:{response.json()}"
            )
        return response.json()
